...
const Danhsach = document.querySelector('.list');
const showListBooks = () => {
    Danhsach.innerHTML= ""
    for(let i = 0; i < Book.length; i++) {
        Danhsach.innerHTML += `
            <li>${i+1}. "${Book[i].title}" by ${Book[i].author}, 
            published in ${Book[i].publishedYear} - ${Book[i].price}.000 VNĐ</li>
        `
    }
};
showListBooks();
...
function addBook(bookList, title, author, publishedYear, price){
