...
bookList[3]={title:"D",author:"g loan",publishedYear:1945,price:20000};
bookList[4]={title:"f",author:"g loan",publishedYear:1945,price:20000};
...
const sumPrice=(bookList=>{
  let sum=0;
  for(let i=0;i<bookList.length;i++){
    sum+=bookList[i].price;
  }
  console.log("Tổng giá tiền của tất cả các cuốn sách la " + sum);
})

const averagePrice = (bookList =>{
  let sum=0;
  for(let i=0;i<bookList.length;i++){
    sum+=bookList[i].price;
  }
  console.log("Giá tiền trung bình của tất cả các cuốn sách la " + sum/bookList.length)
})
...
//   ]
// }
const groupByAuthor = ((bookList) => {
  let newbooklist=bookList;
  let setauthor = new Set();
  const bookauthor = {};
  for(let i=0;i < bookList.length; i++){
    setauthor.add(bookList[i].author)
  }
  for (const x of setauthor.keys()) {
    bookauthor[x]=[];
  }
  for(let j=0;j < newbooklist.length; j++){
    let nametg=newbooklist[j].author;
    delete newbooklist[j].author;
    bookauthor[nametg].push(newbooklist[j])
  }
  return bookauthor;
})
